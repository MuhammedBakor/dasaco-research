...
RUN_ID="sustained-150ue-500ms-$(date +%Y%m%d-%H%M%S)"
RESULTS="$ROOT/results/exploratory-sustained-dasaco/$RUN_ID"
...
mkdir -p "$RESULTS"
mkdir -p "$ROOT/results/exploratory-sustained-dasaco/diagnostic-runs"
...
echo "========================================"
echo "DA-SACO SUSTAINED 150-UE 500-MS EXPERIMENT"
echo "Run ID: $RUN_ID"
echo "========================================"
...
classification=EXPLORATORY_SUSTAINED_RUN
planned_ues=150
...
purpose=verify-new-replica-use-under-sustained-arrival
...
QUALIFICATION="EXPLORATORY_SUSTAINED_RUN"
QUALIFICATION_REASON="sustained-arrival-replica-use-sensitivity"
...
if [ "$QUALIFICATION" = "INVALID_EXPLORATORY_INFRASTRUCTURE_RUN" ]; then
    DESTINATION="$ROOT/results/exploratory-sustained-dasaco/diagnostic-runs/$RUN_ID"
...

{
    ...
    echo "classification=EXPLORATORY_SUSTAINED_RUN"
    echo "included_in_primary_comparison=false"
    echo "purpose=verify-new-replica-use-under-sustained-arrival"
} >> "$RESULTS/summary.txt"

echo "[OK] Sustained-arrival run is experimentally valid"
...
echo
echo "[OK] SUSTAINED 150-UE DA-SACO RUN COMPLETED"
