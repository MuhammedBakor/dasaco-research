...
PACKETRUSHER_CONFIG="$ROOT/configs/packetrusher/dasaco-100ue.yml"
...
mkdir -p "$RESULTS"
mkdir -p "$ROOT/results/final-dasaco/diagnostic-runs"
...
echo "[5] Resetting 100 subscribers"
...
echo "[8] Starting one 100-UE workload"
...
QUALIFICATION="VALID_RUN"
QUALIFICATION_REASON="experiment-completed"

if [ "$COMPLETES" -ge 100 ]; then
    FULL_COMPLETION=true
fi

if     [ "$COMPLETES" -ge 100 ] &&     [ "$REJECTS" -eq 0 ] &&     [ "$AMF_STATUS" -eq 0 ] &&     [ "$UNKNOWN_RAN" -eq 0 ]
then
    PERFECT_SERVICE=true
fi

if [ "$STARTED" -ne 100 ]; then
    INFRASTRUCTURE_FAILURE=true
    QUALIFICATION="INVALID_INFRASTRUCTURE_RUN"
...
if [ "$QUALIFICATION" = "INVALID_INFRASTRUCTURE_RUN" ]; then
    DESTINATION="$ROOT/results/final-dasaco/diagnostic-runs/$RUN_ID"
...
echo "[OK] Run is experimentally valid"
